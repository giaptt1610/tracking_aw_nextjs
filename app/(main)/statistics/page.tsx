  ...
  return (
    <div>
      ...
      <Row gutter={[16, 16]} className="mb-6">
        <Col xs={24} sm={8}><Card><Statistic title="Chi phí" value={totals.totalCost} suffix="VND" formatter={(v) => Number(v).toLocaleString('vi-VN')} styles={{ content: { color: '#ff4d4f' } }} /></Card></Col>
        ...
        <Col xs={24} sm={8}><Card><Statistic title="Lợi nhuận" value={totals.totalProfit} suffix="VND" formatter={(v) => Number(v).toLocaleString('vi-VN')} styles={{ content: { color: totals.totalProfit >= 0 ? '#1677ff' : '#ff4d4f' } }} /></Card></Col>
      </Row>
      <Row gutter={[16, 16]} className="mb-6">
        <Col xs={24} lg={16}>
      ...
      <Card title="Chi tiết sản phẩm">
        <Table dataSource={productStats} rowKey="productId" size="middle" pagination={{ pageSize: 10 }} columns={[
          ...
          { title: 'Số đơn', dataIndex: 'orderCount', key: 'orders', align: 'center' as const, width: 80 },
          ...
          { title: 'Lợi nhuận', dataIndex: 'totalProfit', key: 'profit', align: 'right' as const, defaultSortOrder: 'descend' as const,
